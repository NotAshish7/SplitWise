...
// ─── Detect permanent (non-retryable) OAuth errors ───────────────────────────
function isPermanentAuthError(err) {
  const msg = (err.message || '').toLowerCase();
  return (
    msg.includes('invalid_grant') ||    // token expired / revoked
    msg.includes('invalid_client') ||   // wrong client id / secret
    msg.includes('access_denied') ||    // app unpublished or revoked by user
  ...
  if (process.env.GMAIL_IMAP_REFRESH_TOKEN && process.env.GMAIL_IMAP_CLIENT_ID) {
    try {
      ...
      if (isPermanentAuthError(err)) {
        console.error('❌ IMAP OAuth2 — PERMANENT auth failure:', err.message);
        console.error('   Fix: The refresh token has expired or been revoked.');
        console.error('   → Your Google Cloud OAuth app may be in "Testing" mode (tokens expire in 7 days).');
        console.error('   → Go to Google Cloud Console → APIs & Services → OAuth consent screen → Publish app.');
        console.error('   → Then re-generate GMAIL_IMAP_REFRESH_TOKEN and update your Render env vars.');
        console.error('   → IMAP email watcher is disabled until credentials are updated. SMTP sending still works.');
        return { _permanent: true }; // signal: do not retry
      }
      console.error('❌ IMAP OAuth2 token error (will retry):', err.message);
      return null;
    }
  }
...
async function runImapWatcher() {
  ...
  if (!config) return;                      // transient error → caller decides retry
  if (config._permanent) return;            // permanent auth failure → do NOT retry

  const imapConfig = { imap: config.imap }; // strip internal flags before passing to imap-simple
